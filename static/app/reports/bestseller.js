...
$(function() {
  ...
  $('#rptpreview').on('click', function() {
    ...
    var params = '&site_id={{site_id}}&from_date=' + $('#from_date').val() + '&to_date=' + $('#to_date').val() + '&top_limit=' + $('#top_limit').val();

    if(! branch_id) {
      report_url = '{{rpt_server}}frameset?__report=bestseller_all.rptdesign' + opts + params;
    } else {
      params = params + '&branch_id=' + branch_id;
      report_url = '{{rpt_server}}frameset?__report=bestseller_by_branch.rptdesign' + opts + params;
    }
